...
const ProductDetail = () => {
    ...
    const { addToCart } = useCart(); // Usamos el hook directo
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            ...
            try {
                ...
                // Pre-seleccionar la primera variante (ej: 80x190)
    ...
    const handleAddToCart = (e) => {
        ...
        try {
            // --- CONSTRUCCIÓN DEL ITEM (Ajustado para CartPage) ---
            const itemToCart = {
                id: selectedVariant.id, // ID real de la variante (para backend)
                uniqueId: `${selectedVariant.id}-${Date.now()}`, // ID único para frontend (evita duplicados de key)
                nombre: product.nombre,
                medida: selectedVariant.medida, // <--- Importante para mostrar en Carrito
                precio: currentPrice,
    ...
    return (
        <div className="product-page-container noselect"> 
            ...
            <div className={`toast-notification ${toast.show ? 'show' : ''} ${toast.type === 'error' ? 'error' : ''}`}>
                ...
                {toast.type === 'success' && (
                    /* CAMBIO: Mandamos al Carrito (Paso 1) en lugar de Checkout directo */
            ...
            <style>{`
                ...
                .product-page-container { max-width: 1200px; margin: 0 auto; padding: 120px 20px 60px 20px; font-family: 'Helvetica', sans-serif; position: relative; }
                
                /* Toast */
                .toast-notification { position: fixed; top: 100px; right: 20px; background: white; border-left: 5px solid #00a650; box-shadow: 0 4px 15px rgba(0,0,0,0.15); padding: 15px 20px; border-radius: 4px; display: flex; align-items: center; gap: 20px; z-index: 2000; transform: translateX(120%); transition: transform 0.3s ease-out; }
                ...
                .main-image-box img { width: 100%; height: auto; max-height: 500px; object-fit: contain; }
                .thumbnails-row { display: flex; gap: 10px; margin-top: 15px; overflow-x: auto; }
                .thumb { width: 70px; height: 70px; object-fit: cover; border-radius: 4px; cursor: pointer; opacity: 0.6; border: 2px solid transparent; }
                .thumb.active, .thumb:hover { opacity: 1; border-color: #ffd700; }

                /* Info */
                .title { font-size: 2rem; margin-bottom: 10px; color: #111; line-height: 1.2; }
                ...
                .variant-btn { padding: 10px 20px; border: 1px solid #ccc; background: #fff; cursor: pointer; border-radius: 4px; font-size: 0.9rem; transition: 0.2s; }
                ...
                .qty-selector { display: flex; border: 1px solid #ccc; border-radius: 4px; overflow: hidden; height: 50px; }
                .qty-selector button { width: 50px; border: none; background: #f5f5f5; font-size: 1.4rem; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 2px; }
                .qty-selector span { padding: 0 20px; display: flex; align-items: center; font-weight: bold; font-size: 1.1rem; }
                .btn-add { flex: 1; background: #ffd700; border: none; border-radius: 4px; font-weight: bold; font-size: 1rem; cursor: pointer; transition: 0.2s; }
                ...
                .text-description { line-height: 1.6; color: #444; margin-top: 40px; }
            `}</style>
        </div>
    );
};
