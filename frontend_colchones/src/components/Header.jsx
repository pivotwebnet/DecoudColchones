...
const Header = () => {
    ...
    return (
        // 2. APLICAMOS ESTILOS DINÁMICOS AL HEADER
        <header 
            style={{ 
                position: 'sticky', 
                top: 0, 
                ...
                // Animación suave de la sombra y la altura
                transition: 'all 0.3s ease-in-out',
                boxShadow: isScrolled ? '0 4px 20px rgba(0,0,0,0.1)' : 'none',
                paddingBottom: isScrolled ? '0px' : '0px'
            }}
        >
            ...
            <div 
                style={{ 
                    ...
                    transition: 'all 0.4s ease-in-out', // Suavidad del movimiento
                    overflow: 'hidden',                 // Oculta el contenido al cerrarse
                    
                    // Si hay scroll: altura 0, opacidad 0, sin padding.
                    // Si NO hay scroll: altura máxima suficiente, opacidad 1, padding normal.
                    maxHeight: isScrolled ? '0px' : '50px', 
                    opacity: isScrolled ? 0 : 1,
                    padding: isScrolled ? '0 0' : '8px 0' 
                }}
            >
            ...
            {/* 2. HEADER PRINCIPAL */}
            {/* Animamos el padding para que se achique un poco al bajar */}
            <div style={{ 
                padding: isScrolled ? '30px 0' : '20px 0', 
                transition: 'padding 0.3s ease' 
            }}>
                <div className="container-centered" style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', alignItems: 'center', gap: '20px' }}>
                    {/* ... (Todo el contenido igual     que antes: Logo, Buscador, Iconos) ... */}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '25px' }}>
                         <Link to={user ? "/profile" : "/login"} style={styles.iconBtn} title="Mi Cuenta">
                            ...
                            {user && <span style={{fontSize:'0.85rem', marginLeft:'8px', fontWeight: '600'}}>{user.first_name}</span>}
                        </Link>
...
// ... Tus estilos (styles) siguen igual abajo ...
// Solo asegúrate de copiar los estilos que ya tenías en el mensaje anterior
const styles = {
    ...
    logo: { textDecoration: 'none', color: '#1e3a8a', fontSize: '2rem', fontWeight: 'bold', letterSpacing: '3px', fontFamily: 'serif' },
    navLink: { textDecoration: 'none', color: '#334155', fontWeight: '600', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '1px', transition: 'color 0.2s' },
    iconBtn: { textDecoration: 'none', color: '#334155', fontWeight: '500', display: 'flex', alignItems: 'center', transition: 'color 0.2s' },
    badge: { position: 'absolute', top: '-8px', right: '-10px', backgroundColor: '#e11d48', color: 'white', borderRadius: '50%', width: '18px', height: '18px', fontSize: '0.7rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }
};
